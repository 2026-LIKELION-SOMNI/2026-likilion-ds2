...
from soundfit.models import SoundFitProfile, SoundFitSession
...
class DeleteAllDataView(APIView):
    ...
    def post(self, request, uuid):
        ...
        deleted_counts["soundfit_profile"], _ = SoundFitProfile.objects.filter(user=user).delete()
        deleted_counts["soundfit_sessions"], _ = SoundFitSession.objects.filter(user=user).delete()
